...
export default function ContractPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      role_detail: "BNS / INUS 뮤직 사회자 등록 계약",
      fee: "개별 행사별 별도 협의",
    ...
    const backupPayload = {
      ...
      roleDetail: "BNS / INUS 뮤직 사회자 등록 계약",
      fee: "개별 행사별 별도 협의",
    ...
    fetch("/api/notify-sms", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: form.name, phone: form.phone, contractType: "사회자" }) }).catch(console.error);
    fetch("/api/notify-user-sms", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: form.name, phone: form.phone, contractType: "사회자" }) }).catch(console.error);
    ...
    <main className="container">
      <form className="card" onSubmit={submit}>
        <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", gap:12, marginBottom:12}}>
          ...
          <Link className="btn2" href="/bns">BNS 메인</Link>
        </div>
        ...
        <section className="contractPaper">
          ...
          <h3>제2조 (계약 목적)</h3>
          <p>본 계약은 갑이 진행하는 웨딩 및 각종 행사에 있어 을을 사회 진행 인력으로 등록하고, 향후 개별 요청 시 상호 협의 하에 진행하기 위한 기본 사항을 정함을 목적으로 한다.</p>
          ...
          <ol>
            ...
            <li>을은 사전 협의된 대본과 진행 순서를 숙지하여야 한다.</li>
            <li>을은 행사 분위기에 맞는 진행 태도와 발성을 유지하여야 한다.</li>
          </ol>
        ...
        <button className="btn" style={{width:"100%"}} disabled={saving}>{saving ? "제출 중..." : "사회자 계약서 제출하기"}</button>
      </form>
    </main>
  );
}
