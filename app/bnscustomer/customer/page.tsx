...
export default function CustomerContractPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      bank_info: "우리은행 1002-254-928604 비엔에스뮤직",
      memo,
      signature,
      submit_ip: null,
    ...
    const backupPayload = {
      ...
      name: form.applicantName,
      phone: form.applicantPhone,
      contractType: "비엔에스뮤직 고객",
      eventDate: form.eventDate,
      eventTime: form.eventTime,
      eventPlace: form.eventPlace,
      roleDetail: form.ensemble,
      fee: form.totalAmount || "-",
      bankInfo: "우리은행 1002-254-928604 비엔에스뮤직",
      memo,
      signature,
      ...
      extraFields: {
        ...
        account: "우리은행 1002-254-928604 비엔에스뮤직"
      }
    };
    ...
    fetch("/api/notify-sms", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: form.applicantName, phone: form.applicantPhone, contractType: "비엔에스뮤직 고객 계약서 제출" })
    }).catch(console.error);

    fetch("/api/notify-user-sms", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: form.applicantName, phone: form.applicantPhone, contractType: "비엔에스뮤직 고객", smsBrand: "비엔에스뮤직" })
    }).catch(console.error);
    ...
    <main className="container">
      <form className="card" onSubmit={submit}>
        <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", gap:12, marginBottom:12}}>
          <div>
            <div className="brandPill">비엔에스뮤직</div>
            <h1 className="title">🎼 비엔에스뮤직 고객 계약서</h1>
            <p className="desc">웨딩 연주 계약 정보를 입력한 뒤 휴대폰 인증, 전자서명, 통합 동의를 완료해주세요.</p>
          </div>
          <Link className="btn2" href="/bnscustomer">메인</Link>
        </div>
        ...
        <section className="contractPaper">
          ...
          <h3>제1조 (개요)</h3>
          <p>
            본 계약은 고객의 웨딩 연주를 위해 필요한 제반 사항을 연주업체 비엔에스뮤직에서 제공함에 있어
            계약 당사자인 고객과 비엔에스뮤직의 역할과 의무에 관한 법률관계를 규정하여 상호 원활한 웨딩 연주 진행을 목적으로 한다.
          </p>

          <h3>제2조 (계약사항)</h3>
          <p>연주 일시, 행사 장소, 행사 종류, 연주 편성, 신청자 정보, 신랑·신부 정보, 연주 총금액 및 계약금은 본 전자계약서 입력 내용에 따른다.</p>
          <ul>
            <li>연주 주관: 음악 컨설팅 비엔에스뮤직</li>
            <li>입금 계좌번호: 우리은행 1002-254-928604 비엔에스뮤직</li>
          ...
          <ol>
            ...
            <li>고객이 사전 협의 없이 인터넷 사이트 등에 검증되지 않은 악성 게시글을 올려 비엔에스뮤직에 영업 손실을 줄 경우 보상받을 수 없으며 법적인 제재를 받을 수 있다.</li>
            <li>고객과 비엔에스뮤직은 이상의 계약내용을 성실히 수행할 것을 상호 약속한다.</li>
          </ol>
        </section>
        ...
        <button className="btn" style={{width:"100%"}} disabled={saving}>{saving ? "제출 중..." : "비엔에스뮤직 고객 계약서 제출하기"}</button>
      </form>
    </main>
  );
}
