...
export async function POST(req: Request) {
  ...
  if (!webhookUrl) {
    return NextResponse.json({ ok: true, skipped: true, message: "DRIVE_WEBHOOK_URL 환경변수가 없어 구글드라이브 백업은 건너뛰었습니다.", url: "" });
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ ok: false, message: text, url: "" }, { status: 500 });
    }
