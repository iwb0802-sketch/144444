...
function extractUrl(value: any): string {
  ...
  if (typeof value === "string") {
    try {
      ...
      const normal = value.match(/https:\/\/drive\.google\.com\/file\/d\/[^"\\\s]+/);
      if (normal?.[0]) return normal[0];

      const escaped = value.match(/https:\\\/\\\/drive\.google\.com\\\/file\\\/d\\\/[^"\\\s]+/);
  ...
  if (typeof value === "object") {
    ...
    if (typeof value.driveUrl === "string") return value.driveUrl;
